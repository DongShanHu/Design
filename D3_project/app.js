const countries = [
  ...
  { id: "d3", value: 11, region: "Japen" },
  { id: "d4", value: 6, region: "Taiwan" },
];
...
const x = d3.scaleBand().rangeRound([0, chartSize.width]).padding(0.1);

const y = d3.scaleLinear().range([chartSize.height, 0]);
...
countryItems.append("span").text((country) => country.region);
...
countryItems
  ...
  .on("change", (event) => {
    if (unselectedCountryIds.indexOf(event.target.id) == -1) {
      ...
      unselectedCountryIds = unselectedCountryIds.filter(
